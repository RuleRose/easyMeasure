...
@interface UNICDBOperator () {
    FMDatabaseQueue *_dbqueue;
}
@end

@implementation UNICDBOperator
Singleton_Implementation(UNICDBOperator);
...
- (BOOL)insertData:(BaseModel *)data {
    __block BOOL result = YES;
...
- (void)batchInsertData:(NSArray *)dataArray {
    ...
    [_dbqueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
      BOOL result = NO;
      for (BaseModel *data in dataArray) {
          NSArray *cols = [data transToDictionary].allKeys;
...
- (BOOL)deleteData:(BaseModel *)data dependOnKeys:(NSArray *)keys{
    __block BOOL result = YES;
    [_dbqueue inDatabase:^(FMDatabase *db) {
...
- (BOOL)updateData:(BaseModel *)data dependOnKeys:(NSArray *)keys {
    __block BOOL result = YES;
    [_dbqueue inDatabase:^(FMDatabase *db) {
...
- (BOOL)clearTable:(BaseModel *)data {
    __block BOOL result = YES;
...
- (NSString *)tableNameForModel:(BaseModel *)model {
