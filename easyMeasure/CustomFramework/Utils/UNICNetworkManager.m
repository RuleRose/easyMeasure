...
#import "UNICHUDManager.h"
#import "UNICServerManager.h"

#define NetworkMethodList @[ @"Get", @"Post", @"Put", @"Delete" ]

static UNICNetworkManager *_shareManager = nil;
static NSURL *_baseurl = nil;
// static NSString *_acesstoken = nil;

@implementation UNICNetworkManager
+ (UNICNetworkManager *)shareManager {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
      UNICServerManager *serverManager = [UNICServerManager shareManager];
      ...
      _shareManager = [[UNICNetworkManager alloc] initWithBaseURL:_baseurl];
...
+ (instancetype)networkManager {
    UNICNetworkManager *networkMgr = [[self alloc] initWithBaseURL:_baseurl];
...
- (NSURLSessionDataTask *)requestWithPath:(NSString *)path
    ...
    switch (method) {
        ...
        case POST: {
            ...
            //                  block(nil, error);
            //                }];
            NSString *reqURL = [NSString stringWithFormat:@"http://%@", [UNICServerManager shareManager].serverURL];
                       ...
                       if (!error) {
                           if ([responseObject isKindOfClass:[NSDictionary class]]) {
                               if ([[responseObject leie_getObjectByPath:@"result"] integerValue] == 0) {
                                   ...
                                   block(responseObject, [UNICNetworkManager shareManager].errorDataNotDiction);
                               }
                           } else {
                               block(nil, [UNICNetworkManager shareManager].errorDataNotDiction);
                           }
                       } else {
                           [[UNICHUDManager defaultInstance] showTextHUD:kLocalization(@"alert_common_request_failed")];
                           DDLogDebug(@"\n===========response===========\n%@:\n%@", path, error);
...
- (id)handleResponse:(id)responseJSON autoShowError:(BOOL)autoShowError {
    ...
    if (status.intValue != 0) {
        error = [NSError errorWithDomain:[UNICServerManager shareManager].serverURL code:status.intValue userInfo:responseJSON];
    }
    return error;
}
@end
